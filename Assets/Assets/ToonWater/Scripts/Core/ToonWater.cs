...
public class ToonWater:MonoBehaviour{
    ...
    public static ParticleSystem _splashPS = null;
    public static ParticleSystem _ripplePS = null;
    ...
    public void initFloatObject(ToonWaterFloatObject fo){
			if(fo.GetComponent<Rigidbody>() == null)
				return;
    		fo.GetComponent<Rigidbody>().linearDamping =(UnityEngine.Random.value*.5f+.3f);		//Adds drag to simulate movement restrictions in water
    		if(!fo.initialized){
    			fo.initialized = true;
    			if(ripplePS != null)
	...
	private void OnDisable() {
		...
		{
            _splashPS.Stop();
			Destroy(_splashPS.gameObject);
			_splashPS = null;
		}
        if(_ripplePS != null)
		{
            _ripplePS.Stop();
			Destroy(_ripplePS.gameObject);
			_ripplePS = null;
		}
	}
